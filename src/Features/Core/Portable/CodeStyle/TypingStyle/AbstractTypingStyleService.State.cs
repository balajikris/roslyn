...
{
    internal partial class AbstractTypingStyleService<TService>
    {
        [Flags]
        ...
        private class State
        {
            ...
            public State(TService service, bool isVariableDeclarationContext)
            {
                _service = service;
            ...
            public static State Generate(TService service, SyntaxNode declaration, SemanticModel semanticModel, OptionSet optionSet, bool isVariableDeclarationContext, CancellationToken cancellationToken)
            {
                var state = new State(service, isVariableDeclarationContext);
            ...
            private void Initialize(SyntaxNode declaration, SemanticModel semanticModel, OptionSet optionSet, CancellationToken cancellationToken)
            {
                ...
                IsTypingApparentInContext = false;
                /*    IsInVariableDeclarationContext
                        ? IsTypeApparentInDeclaration((TVariableDeclarationSyntax)declaration,
            ...
            /// </summary>
            private bool IsTypeApparentInDeclaration(SyntaxNode variableDeclaration, SemanticModel semanticModel, TypingStyles stylePreferences, CancellationToken cancellationToken)
            {
            ...
            private bool IsIntrinsicType(SyntaxNode declarationStatement) => false;
            ...
            private SyntaxNode GetInitializerExpression(SyntaxNode initializer) => initializer;
           /*     initializer.Value is CheckedExpressionSyntax
                    ? ((CheckedExpressionSyntax)initializer.Value).Expression
                    : initializer.Value;
                    */

            private bool IsPossibleCreationOrConversionMethod(IMethodSymbol methodSymbol, ITypeSymbol declaredType, SemanticModel semanticModel, SyntaxNode typeName, CancellationToken cancellationToken)
            {
